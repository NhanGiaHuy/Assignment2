...
public class TronMapSurvival extends TronMap {
	...
	public JPanel getHighs() {
		...
		JLabel j0 = new JLabel("    One Player Mode High Scores: ");
		j0.setForeground(Color.WHITE);
		j0.setBackground(Color.BLACK);
		j0.setFont(new Font("Copperplate Gothic Bold", Font.PLAIN, 40));
		panel.add(j0);
		...
		for (int i = 0; i < 5; i++)
		{
			...
			j1.setFont(new Font("Copperplate Gothic Bold", Font.PLAIN, 40));
			panel.add(j1);

			JLabel j6 = new JLabel("      " + right + ".) " + highScores.get(right - 1).toString());
			j6.setForeground(Color.WHITE);
			j6.setBackground(Color.BLACK);
			j6.setFont(new Font("Copperplate Gothic Bold", Font.PLAIN, 40));
			panel.add(j6);
			left++;
			right++;
		}
